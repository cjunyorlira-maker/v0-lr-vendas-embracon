...
export default function AdicionarUsuarioModal({
  ...
  const rolesPermitidos = TODOS_ROLES.filter((r) => {
    if (currentUserRole === 'master') return r.value === 'representante'
  ...
  function handleLogoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setError('Logo muito grande (máx 2MB)')
      return
    }
  ...
  const mostraEmpresa = currentUserRole === 'master' && role === 'representante'
  const mostraEquipe = ['supervisor', 'vendedor'].includes(role) && currentUserRole !== 'master'
        ...
        {senhaGerada ? (
          <div className="space-y-4">
            <p className="text-sm" style={{ color: 'var(--text2)' }}>
              Passe esses dados pro novo usuário. <strong>Não serão mostrados novamente.</strong>
            </p>
            ...
            <div>
              ...
              <div className="flex items-center justify-between rounded-lg p-3" style={{ background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.2)' }}>
                ...
                <button onClick={() => copyValue(emailCriado!, 'email')} className="p-1.5 rounded" style={{ color: copiedEmail ? '#22c55e' : 'var(--accent)' }} title="Copiar email">
            ...
            <div>
              ...
              <div className="flex items-center justify-between rounded-lg p-3" style={{ background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.2)' }}>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => copyValue(senhaGerada!, 'senha')} className="p-1.5 rounded" style={{ color: copiedSenha ? '#22c55e' : 'var(--accent)' }} title="Copiar senha">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select value={role} onChange={(e) => setRole(e.target.value)} className="w-full rounded-lg px-3 py-2.5 text-sm outline-none" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', color: 'var(--text)' }}>
                {rolesPermitidos.map((r) => (
                  <option key={r.value} value={r.value} style={{ background: '#131313' }}>{r.label}</option>
                ))}
              </select>
            </div>
            ...
            {mostraEmpresa && (
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--muted-color)' }}>
                  Nome da empresa do representante <span style={{ color: '#ef4444' }}>*</span>
                </label>
            ...
            {mostraEmpresa && (
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: 'var(--muted-color)' }}>
                  Logo da empresa <span style={{ color: 'var(--muted-color)' }}>(opcional)</span>
                </label>
                ...
                {!logoPreview ? (
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full rounded-lg p-4 flex flex-col items-center gap-2 cursor-pointer" style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed var(--border)', color: 'var(--muted-color)' }}>
                    <Upload size={20} />
                    <span className="text-xs">Selecionar imagem (máx 2MB)</span>
                  </button>
                ) : (
                  <div className="rounded-lg p-3 flex items-center gap-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)' }}>
                    <img src={logoPreview} alt="Preview" className="h-12 w-12 object-contain rounded" />
                    <div className="flex-1">
                      <p className="text-xs" style={{ color: 'var(--text2)' }}>Logo carregada</p>
                      <button type="button" onClick={removerLogo} className="text-xs underline mt-1" style={{ color: '#ef4444' }}>Remover</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            <button type="submit" disabled={loading} className="w-full rounded-lg py-2.5 text-sm font-semibold disabled:opacity-50" style={{ background: 'linear-gradient(135deg, #d4af37 0%, #c9a227 50%, #b8941f 100%)', color: '#0a0a0a' }}>
