...
export default function NovaVendaPage() {
  ...
  async function handleSalvar() {
    ...
    if (!dataAssembleia) {
      setErro('Informe a data da assembleia de entrada. Digite o grupo e aguarde a busca (✓ Grupo mapeado), ou preencha a data manualmente.')
      setSalvando(false)
      return
    }
    try {
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl p-5" style={{ background: 'rgba(17,18,22,0.92)', boxShadow: '0 8px 24px rgba(0,0,0,0.45)', border: '1px solid var(--border)' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div><label className="block text-xs mb-1" style={{ color: 'var(--muted-color)' }}>Grupo</label><input value={grupo} onChange={(e) => setGrupo(e.target.value)} onBlur={(e) => buscarGrupo(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle} />{buscandoGrupo && <p className="text-xs mt-1" style={{ color: 'var(--muted-color)' }}>Buscando...</p>}{grupoEncontrado === true && dataAssembleia && <p className="text-xs mt-1" style={{ color: '#22c55e' }}>{'\u2713'} Grupo mapeado</p>}{grupoEncontrado === true && !dataAssembleia && <p className="text-xs mt-1" style={{ color: '#f59e0b' }}>{'\u26a0'} Grupo sem calendário — informe a assembleia manualmente</p>}{grupoEncontrado === false && <p className="text-xs mt-1" style={{ color: '#f59e0b' }}>{'\u26a0'} Grupo novo — informe a assembleia</p>}</div>
