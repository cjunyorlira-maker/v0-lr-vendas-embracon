...
export default function NovaVendaPage() {
  ...
  // Atribuição (empresa/equipe/vendedor)
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      // carrega opções de atribuição
      try {
        const res = await fetch('/api/vendas/opcoes')
        const op = await res.json()
        if (op.meu_role) {
          setMeuRole(op.meu_role)
          setIsMatriz(op.is_matriz)
          setEmpresas(op.empresas || [])
          setEquipes(op.equipes || [])
          setVendedores(op.vendedores || [])
        }
      } catch {}
    })
  }, [])
  ...
  function calcularProximaCobranca() {
    ...
    if (noCalendario && alvo.getFullYear() === 2026) return noCalendario.data_vencimento
  ...
  async function buscarGrupo(grupoNum: string) {
    ...
    try {
      ...
      if (data.encontrado) {
        ...
        setGrupoEncontrado(false); setCalendarioGrupo([])
      }
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (data.parse_falhou) {
        setAvisoParser(data.error || 'Não consegui ler o PDF. Preencha manualmente.')
      } else if (data.dados) {
        const d = data.dados
  ...
  function parseValor(s: string): number { return parseFloat(s.replace(/\./g, '').replace(',', '.')) || 0 }

  function limparForm() {
    setNomeCliente(''); setCpfCnpj(''); setTelefone(''); setEmail('')
    setNumeroProposta(''); setNumeroContrato(''); setGrupo(''); setCota('')
    setValorCredito(''); setValorPrimeiraParcela(''); setValorDemaisParcelas('')
    setAdesaoPercent(''); setPlanoId(''); setQtdParcelas('0'); setObservacoes('')
    setDataAssembleia(''); setDataVencimento(''); setCalendarioGrupo([]); setProximaCobranca('')
    setGrupoEncontrado(null); setPdfBase64(null); setPdfNome(''); setErro(''); setAvisoParser('')
  ...
  async function handleSalvar() {
    ...
    try {
      const body: any = {
        ...
        numero_proposta: numeroProposta || null, numero_contrato: numeroContrato || null,
        grupo: grupo || null, cota: cota || null,
        ...
        pdf_base64: pdfBase64, pdf_nome: pdfNome,
        observacoes: observacoes || null,
        data_assembleia_entrada: dataAssembleia || null,
        proxima_cobranca: proximaCobranca || null,
        empresa_id_alvo: empresaSel || null,
        equipe_id_alvo: equipeSel || null,
        vendedor_id_alvo: vendedorSel || null,
      }
      if (grupoEncontrado === false && grupo && dataAssembleia) {
        try {
          await fetch(`/api/grupos/${grupo}`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ bem: planos.find(p => p.id === planoId)?.bem || null, data_assembleia: dataAssembleia, dia_vencimento: null }) })
        } catch {}
      }
  ...
  // filtra equipes/vendedores pela empresa selecionada (matriz)
  const equipesFiltradas = isMatriz && empresaSel ? equipes.filter(e => e.empresa_id === empresaSel) : equipes
  const vendedoresFiltrados = vendedores.filter(v => {
    if (isMatriz && empresaSel && v.empresa_id !== empresaSel) return false
    if (equipeSel && v.equipe_id !== equipeSel) return false
    return true
  })
        ...
        <main className="mx-auto max-w-3xl px-6 py-8 lg:px-8">
          ...
          {etapa === 'upload' && (
            <div className="rounded-xl p-8" style={{ background: 'rgba(0,0,0,0.12)', backdropFilter: 'blur(4px)', border: '1px solid var(--border)' }}>
              ...
              <div onClick={() => fileInputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f) }}
                className="flex flex-col items-center justify-center gap-3 py-12 rounded-xl cursor-pointer" style={{ border: '2px dashed var(--border)', background: 'rgba(255,255,255,0.02)' }}>
                {parsing ? (<><Loader2 size={40} className="animate-spin" style={{ color: 'var(--accent)' }} /><p className="text-sm" style={{ color: 'var(--text2)' }}>Lendo a proposta...</p></>
                ) : (<><div className="flex h-14 w-14 items-center justify-center rounded-full" style={{ background: 'rgba(212,175,55,0.15)' }}><Upload size={24} style={{ color: 'var(--accent)' }} /></div>
                  <p className="text-base font-medium" style={{ color: 'var(--text)' }}>Solte a proposta em PDF aqui</p>
                  <p className="text-xs" style={{ color: 'var(--muted-color)' }}>ou clique para selecionar · leitura automática</p></>)}
              </div>
          ...
          {etapa === 'sucesso' && (
            <div className="flex flex-col items-center justify-center py-16 gap-5 rounded-xl" style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)' }}>
              <div className="flex h-20 w-20 items-center justify-center rounded-full animate-bounce" style={{ background: 'rgba(34,197,94,0.15)' }}>
                <PartyPopper size={40} style={{ color: '#22c55e' }} />
              </div>
              <div className="text-center">
                <h2 className="text-2xl font-bold mb-1" style={{ color: 'var(--text)' }}>Parabéns!</h2>
                <p className="text-sm" style={{ color: 'var(--text2)' }}>Venda de <strong>{nomeCliente}</strong> cadastrada com sucesso!</p>
                <p className="text-xs mt-1" style={{ color: 'var(--muted-color)' }}>O boleto entrou como pendente para a administração solicitar.</p>
              </div>
              <div className="flex gap-3 mt-2">
                <button onClick={() => { limparForm(); setEtapa('upload') }} className="flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold transition-transform hover:scale-105 active:scale-95" style={{ background: 'linear-gradient(135deg, #d4af37 0%, #c9a227 50%, #b8941f 100%)', color: '#0a0a0a' }}>
                  <Plus size={16} />Cadastrar próxima venda
                </button>
                <button onClick={() => router.push('/clientes')} className="rounded-lg px-5 py-2.5 text-sm transition-transform hover:scale-105 active:scale-95" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', color: 'var(--text2)' }}>
                  Ver clientes
                </button>
              </div>
            </div>
          ...
          {etapa === 'revisar' && (
            <div className="space-y-5">
              {pdfNome && (<div className="flex items-center gap-2 rounded-lg p-3" style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)' }}>
                <FileText size={16} style={{ color: '#22c55e' }} /><span className="text-sm" style={{ color: 'var(--text2)' }}>{pdfNome}</span>
                <button onClick={() => { setEtapa('upload'); setPdfNome(null); setPdfBase64(null) }} className="ml-auto text-xs" style={{ color: 'var(--muted-color)' }}>Trocar</button></div>)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {isMatriz && (
                      <div>
                        ...
                        <select value={empresaSel} onChange={(e) => { setEmpresaSel(e.target.value); setEquipeSel(''); setVendedorSel('') }} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle}>
                          <option value="" style={{ background: '#131313' }}>Minha empresa</option>
                    ...
                    {['master','representante','adm'].includes(meuRole) && (
                      <div>
                        ...
                        <select value={equipeSel} onChange={(e) => { setEquipeSel(e.target.value); setVendedorSel('') }} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle}>
                          <option value="" style={{ background: '#131313' }}>Sem equipe</option>
                          {equipesFiltradas.map(e => <option key={e.id} value={e.id} style={{ background: '#131313' }}>{e.nome}</option>)}
                        </select>
                      </div>
                    )}
                    <div>
                      <label className="block text-xs mb-1" style={{ color: 'var(--muted-color)' }}>Vendedor</label>
                      <select value={vendedorSel} onChange={(e) => setVendedorSel(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle}>
                        <option value="" style={{ background: '#131313' }}>Eu mesmo</option>
                        {vendedoresFiltrados.map(v => <option key={v.id} value={v.id} style={{ background: '#131313' }}>{v.nome}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button onClick={handleSalvar} disabled={salvando} className="flex items-center gap-2 rounded-lg px-6 py-2.5 text-sm font-semibold disabled:opacity-50 transition-transform hover:scale-105 active:scale-95" style={{ background: 'linear-gradient(135deg, #d4af37 0%, #c9a227 50%, #b8941f 100%)', color: '#0a0a0a' }}>
